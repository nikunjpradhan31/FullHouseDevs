...
# Note press Q to stop the demo, press S to toggle confidence
...
configuration_dict = {
    "synthetic": {
        "model_path": MODELS_DIR / "yolov8m_synthetic.pt",
        "class_names": [
            "10c", "10d", "10h", "10s",
            "2c", "2d", "2h", "2s",
            "3c", "3d", "3h", "3s",
...
if configuration_model not in configuration_dict.keys():
    print(f"Allowed parameters for model are {list(configuration_dict.keys())}. Defaulting to {DEFAULT_MODEL}...")
...
# Start webcam
# WSL FIX 1: Explicitly use V4L2 backend and MJPG codec for stable USB bridge streaming
cap = cv2.VideoCapture(0, cv2.CAP_V4L2)
cap.set(cv2.CAP_PROP_FOURCC, cv2.VideoWriter_fourcc(*'MJPG'))

# Set width and height (using named properties is a bit safer/cleaner than numbers 3 and 4)
...
while True:
    ...
    # WSL FIX 2: Safety check to prevent OpenCV crashes if a frame drops
    ...
    # KEYBOARD FIX: Capture the key press once per frame
